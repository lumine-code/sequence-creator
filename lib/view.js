...
module.exports = class SequentialNumberView extends Emitter {
  constructor() {
    super();
    const document = lumine.workspace.getPrimaryWindowSurface().document;
  ...
  show(owner) {
    ...
    lumine.workspace.rehomeModalPanel(this.modalPanel, { owner });
  ...
  hide() {
    if (this.refreshSub) this.unbindEvents();
  ...
  destroy() {
    ...
    this.element.remove();
    this.textEditor.getModel().destroy();
    this.dispose();
    this.element = null;
  }
